...
export const Route = createFileRoute('/organizer/courses_/$courseId/attendees')({
...
function OrganizerCourseAttendeesPage() {
  ...
  const filtered = data.attendees.filter((attendee) => {
    const matchesSearch =
      search.trim() === '' ||
      attendee.learnerName.toLowerCase().includes(search.toLowerCase()) ||
      attendee.learnerEmail.toLowerCase().includes(search.toLowerCase())
    const matchesStatus =
    ...
    <div className="page-stack">
      ...
      <section className="metrics-grid">
        <article className="metric-card">
          <p>Total</p>
          <strong>{data.attendees.length}</strong>
        </article>
        <article className="metric-card">
          <p>Active</p>
        ...
        <article className="metric-card">
          <p>Completed</p>
          <strong>{data.stats.completed + data.stats.confirmed}</strong>
        </article>
      </section>
            ...
            {filtered.map((attendee) => (
              <article className="attendee-row" key={attendee.id}>
                <div
                  ...
                  style={
                    { '--attendee-accent': data.course.accent } as React.CSSProperties
                  }
                >
                ...
                <div className="attendee-meta">
                  <span className={`pill attendee-status ${STATUS_CLASS[attendee.status] ?? ''}`}>
                    {attendee.status}
                  </span>
