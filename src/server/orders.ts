...
function buildMetrics(orderRows: OrderRow[]) {
  ...
  const totalRevenue = confirmed.reduce((sum, order) => sum + order.amountPaid, 0)
  const thisMonthStart = new Date()
  thisMonthStart.setDate(1)
  thisMonthStart.setHours(0, 0, 0, 0)
  const thisMonth = confirmed.filter(
    (order) => new Date(order.createdAt).getTime() >= thisMonthStart.getTime(),
  )
...
function getOrganizerCourseIds(organizerId: number) {
...
export function getOrganizerOrders() {
  ...
  const database = bootstrapDatabase()
  const organizerCourseIds = getOrganizerCourseIds(organizer.id)
